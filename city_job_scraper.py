...
def get_job_links(some_html):
    """get_job_links works on a single webpage.
       It returns a list of urls to the current job postings on the City of Austin's website. It
       takes a raw html string as a argument and parses it using BeautifulSoup """
    ...
    links_soup = BeautifulSoup(
        some_html,
        'html.parser',
        parse_only=SoupStrainer('td', {"class": "job-title"}))
...
def get_headers(table_rows):
    """get_headers gathers only the th tags from an html table to set the
       headers for a csv export"""
...
def build_dataframe(links):
    ...
    # look at first link only for headers
    headers = get_headers(get_tables(links[0]))
